...
import { ICompoundApprovalStatus } from "../Model/CompoundApprovalStatus";
...
export const ApprovalStatusAsync = async(erc20Token:string, txAddress:string) : Promise<ICompoundApprovalStatus>=> {

  // Setting up Ethereum blockchain Node through Infura
  //console.log("Infura URL:",process.env.infuraUrl)
  const web3 = new Web3(process.env.infuraUrlRinkeby!);
  //web3.setProvider(new Web3.providers.HttpProvider("http://localhost:4200"));
...
if(erc20Token === RNKBYDAI){
   ...
   underlyingContract = new web3.eth.Contract(cTokenAbiJson as AbiItem[], RNKBYcDAI);
...
}else if(erc20Token === RNKBYBAT)
{
   ...
   underlyingContract = new web3.eth.Contract(cTokenAbiJson as AbiItem[], RNKBYcBAT);
...
}else if(erc20Token === RNKBYETH)
{
  ...
  underlyingContract = new web3.eth.Contract(cTokenAbiJson as AbiItem[], RNKBYcETH);
...
}else if(erc20Token === RNKBYREP)
{
  ...
  underlyingContract = new web3.eth.Contract(cTokenAbiJson as AbiItem[], RNKBYcREP);
...
{    
  ...
  underlyingContract = new web3.eth.Contract(cTokenAbiJson as AbiItem[], RNKBYcUSDC);
...
}else if(erc20Token  === RNKBYUSDT)
{
  ...
  underlyingContract = new web3.eth.Contract(cTokenAbiJson as AbiItem[], RNKBYcUSDT);
...
}else if(erc20Token  === RNKBYWBTC)
{
  ...
  underlyingContract = new web3.eth.Contract(cTokenAbiJson as AbiItem[], RNKBYcWBTC);
...
}else if(erc20Token  === RNKBYZRX)
{
  ...
  underlyingContract = new web3.eth.Contract(cTokenAbiJson as AbiItem[], RNKBYcZRX);
  underlyingContractAddress=RNKBYZRX;
    ...
    try {
        ...
        //ERC20Interface erc20Contract = ERC20Interface(underlyingContract);
        
        tokenAllowanceValue = await underlyingContract.methods.allowance(txAddress,txAddress).call();
        
        //let encoded_tx = underlyingContract.methods.allowance(txAddress,txAddress).encodeABI();
        /*
        ...
        */
          ...
          let compoundApprovalStatus:ICompoundApprovalStatus = {
            tokenAllowance: tokenAllowanceValue,
            underlyingTokenContract:underlyingContractAddress,
            compoundTokenContract:cTokenContractAddress,
                }
  return compoundApprovalStatus;
